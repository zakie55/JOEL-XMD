...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    if (!isBotAdmins) {
      return m.reply('*вσт мυѕт вє αη α∂мιη тσ υѕє тнιѕ ¢σммαη∂.*');
    }

    if (!text) return m.reply(`*єηтєя тнє ηυмвєя уσυ ωαηт тσ ιηνιтє тσ тнє gяσυρ*\n ηєχαмρℓє:\n*${PREFIX + cmd}* 255714595078`);
    if (text.includes('+')) return m.reply(`*єηтєя тнє ηυмвєя тσgєтнєя ωιтнσυт *+*`);
    if (isNaN(text)) return m.reply(`*єηтєя σηℓу тнє ηυмвєяѕ ρℓυѕ уσυя ¢συηтяу ¢σ∂є ωιтнσυт ѕρα¢єѕ`);
